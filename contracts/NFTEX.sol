...
contract NFTEX is ERC721Holder, Ownable {
    ...
    mapping(IERC721Metadata => mapping(uint256 => bytes32[]))
    ...
    constructor(address tokenERC20, uint16 _feePercent) {
    ...
    function _makeOrder(
        ...
        uint256 _royaltyFeePercent = IAnconNFT(address(_token)).getRoyaltyFee(
            _id
        );
    ...
    function buyItNow(bytes32 _order) external payable {
        ...
        require(
            o.orderType == OrderType.Fixed,
            "Invalid order type, Its not a fix price order"
        );
        ...
        require(
            nativeCoin.allowance(msg.sender, address(this)) >= currentPrice,
            "Balance not allowed"
        );
...
abstract contract IAnconNFT {
    function getCreator(uint256 id) external view virtual returns (address);

    function getRoyaltyFee(uint256 id) external view virtual returns (uint256);
}
